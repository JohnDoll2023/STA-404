...
# MacBook:  /Users/baileraj/Desktop/Classes/sta404-fall-2020
...
# mutate -- adds columns/ new variable
#
# dyanmite plot =========================================================
...
myDF %>% 
  ggplot(aes(x=fgroup, color=fgroup)) +
  geom_bar()
...
ggplot(data = labDF, aes(x = as.factor(year), y = lifeExp, group = year)) +
  geom_violin() +
...
ggplot() +
  geom_violin(data=labDF, aes(x=as.factor(year), y=lifeExp))
#  5. superimpose violin plot on the jittered points
ggplot(data=labDF, aes(x=as.factor(year), y=lifeExp)) +
  geom_violin() +
  geom_jitter(alpha = 0.4)  
...
#The average age for citizens of countries in Asia is increasing
# ADDENDUM
